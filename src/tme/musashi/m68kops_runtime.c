...
typedef struct {
    m68ki_instruction_jump_call opcode_handler;
    unsigned int mask;
    unsigned int match;
...
m68ki_dispatch_entry *m68ki_dispatch_table;

static inline void set_dispatch(unsigned int opcode,
                                const opcode_handler_struct *descriptor) {
    m68ki_dispatch_table[opcode].handler = descriptor->opcode_handler;
    m68ki_dispatch_table[opcode].cycles = descriptor->cycles[0];
}
...
void m68ki_build_opcode_table(void) {
    ...
    if (m68ki_dispatch_table != NULL) {
        return;
    }

    m68ki_dispatch_table = tme_psram_aligned_alloc(
        0x10000 * sizeof(*m68ki_dispatch_table),
        MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (m68ki_dispatch_table == NULL) {
        fputs("Failed to allocate 68K dispatch table in PSRAM\n", stderr);
        abort();
    }

    for (i = 0; i < 0x10000; ++i) {
        m68ki_dispatch_table[i].handler = m68k_op_illegal;
        m68ki_dispatch_table[i].cycles = 0;
    }

    const opcode_handler_struct *descriptor = m68k_opcode_handler_table;
    while (descriptor->mask != 0xff00) {
        for (i = 0; i < 0x10000; ++i) {
            if ((i & descriptor->mask) == descriptor->match) {
                set_dispatch(i, descriptor);
            }
        }
        ++descriptor;
    }
    while (descriptor->mask == 0xff00) {
        for (i = 0; i <= 0xff; ++i) {
            set_dispatch(descriptor->match | i, descriptor);
        }
        ++descriptor;
    }
    while (descriptor->mask == 0xf1f8) {
        for (i = 0; i < 8; ++i) {
            for (j = 0; j < 8; ++j) {
                instr = descriptor->match | (i << 9) | j;
                set_dispatch(instr, descriptor);
                if ((instr & 0xf000) == 0xe000 && !(instr & 0x20)) {
                    cycle_cost = ((((i - 1) & 7) + 1) << 1);
                    m68ki_dispatch_table[instr].cycles += cycle_cost;
                }
            }
        }
        ++descriptor;
    }
    while (descriptor->mask == 0xfff0) {
        for (i = 0; i <= 0x0f; ++i) {
            set_dispatch(descriptor->match | i, descriptor);
        }
        ++descriptor;
    }
    while (descriptor->mask == 0xf1ff) {
        for (i = 0; i <= 0x07; ++i) {
            set_dispatch(descriptor->match | (i << 9), descriptor);
        }
        ++descriptor;
    }
    while (descriptor->mask == 0xfff8) {
        for (i = 0; i <= 0x07; ++i) {
            set_dispatch(descriptor->match | i, descriptor);
        }
        ++descriptor;
    }
    while (descriptor->mask == 0xffff) {
        set_dispatch(descriptor->match, descriptor);
        ++descriptor;
    }
}
